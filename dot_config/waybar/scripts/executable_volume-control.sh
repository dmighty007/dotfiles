...
notify_mute() {
  mute=$(wpctl get-sink-mute @DEFAULT_SINK@ | awk '{print $2}')
  if [ "${mute}" = "yes" ]; then
...
action_volume() {
  ...
  i)
    ...
    current_vol=$(wpctl get-volume @DEFAULT_SINK@ | awk '{print $2}' | sed 's/%//')
    if (($(echo "$current_vol < 1.0" | bc -l) )); then
...
i) action_volume i ;;
d) action_volume d ;;
m) wpctl set-mute @DEFAULT_SINK@ toggle && exit 0 ;;
*) print_error ;;
esac
